<template>
	<view class="regBg">
		<view class="regBox" v-if="pageLoad">
			...
			<button type="primary" form-type="submit" class="btn-row-submit btn-success">手机登录</button>
	...
	export default{
		...
		methods:{
			...
			getYzm:function(){
					...
					var that=this;
					uni.request({
						url:that.app.apiHost+"?m=loginphone&a=SendSms&ajax=1",
						data:{
			...
			formSubmit:function(e){
				...
				uni.request({
					url:that.app.apiHost+"?m=loginphone&a=save&ajax=1",
					method:"POST",
					header:{
